...
(function(window, document, $, undefined){
	...
	window.CMB = {
		...
		init: function() {
			CMB.log( window.whizzy_l10 );

			// hide our spinner gif if we're on a MP6 dashboard
			if ( window.whizzy_l10.new_admin_style ) {
			 ...
			 */
			$('.whizzy_timepicker').each( function() {
			 ...
			 */
			$('.whizzy_datepicker').each( function() {
			...
			$("#ui-datepicker-div").wrap('<div class="whizzy_element" />');

			/**
			 * Initialize color picker
			 */
			if (typeof jQuery.wp === 'object' && typeof jQuery.wp.wpColorPicker === 'function') {
				$('input:text.whizzy_colorpicker').wpColorPicker();
			} else {
				$('input:text.whizzy_colorpicker').each( function(i) {
			...
			$('.whizzy_id__whizzy_file')
			.on( 'change', '.whizzy_upload_file', function() {
				CMB.formfield = $(this).attr('name');
				$('#' + CMB.formfield + '_id').val("");
			})
			.on( 'click', '.whizzy_upload_button', function(event) {
				...
				var attachment = true;
				var isList = $self.hasClass( 'whizzy_upload_list' );
				...
				CMB.file_frames[CMB.formfield] = wp.media.frames.file_frame = wp.media({
					...
					button: {
						text: window.whizzy_l10.upload_file
					},
				...
				CMB.file_frames[CMB.formfield].on( 'select', function() {
					...
					if ( isList ) {
						...
						$( attachment ).each( function() {
							if ( this.type && this.type === 'image' ) {
								...
								uploadStatus = '<li class="img_status">'+
									...
									'<p><a href="#" class="whizzy_remove_file_button" rel="'+ CMB.formfield +'['+ this.id +']">'+ whizzy_l10.remove_image +'</a></p>'+
									'<input type="hidden" id="filelist-'+ this.id +'" name="'+ CMB.formfield +'['+ this.id +']" value="' + this.url + '">'+
								'</li>';

							} else {
								// Standard generic output if it's not an image.
								uploadStatus = '<li>'+ whizzy_l10.file +' <strong>'+ this.filename +'</strong>&nbsp;&nbsp;&nbsp; (<a href="' + this.url + '" target="_blank" rel="external">'+ whizzy_l10.download +'</a> / <a href="#" class="whizzy_remove_file_button" rel="'+ CMB.formfield +'['+ this.id +']">'+ whizzy_l10.remove_file +'</a>)'+
									'<input type="hidden" id="filelist-'+ this.id +'" name="'+ CMB.formfield +'['+ this.id +']" value="' + this.url + '">'+
								'</li>';
						...
						// Append each item from our fileGroup array to .whizzy_media_status
						$( fileGroup ).each( function() {
							$formfield.siblings('.whizzy_media_status').slideDown().append(this);
						});
						...
						if ( attachment.type && attachment.type === 'image' ) {
							// image preview
							uploadStatus = '<div class="img_status"><img style="max-width: 350px; width: 100%; height: auto;" src="' + attachment.url + '" alt="'+ attachment.filename +'" title="'+ attachment.filename +'" /><p><a href="#" class="whizzy_remove_file_button" rel="' + CMB.formfield + '">'+ whizzy_l10.remove_image +'</a></p></div>';
						} else {
							// Standard generic output if it's not an image.
							uploadStatus = whizzy_l10.file +' <strong>'+ attachment.filename +'</strong>&nbsp;&nbsp;&nbsp; (<a href="'+ attachment.url +'" target="_blank" rel="external">'+ whizzy_l10.download +'</a> / <a href="#" class="whizzy_remove_file_button" rel="'+ CMB.formfield +'">'+ whizzy_l10.remove_file +'</a>)';
						}

						// add/display our output
						$formfield.siblings('.whizzy_media_status').slideDown().html(uploadStatus);
					}
				});
			...
			.on( 'click', '.whizzy_remove_file_button', function(event) {
				event.preventDefault();
				var $self        = $(this);
				if ( $self.is( '.attach_list .whizzy_remove_file_button' ) ){
				...
				if ( ! $container.length ) {
					$self.parents('.whizzy_media_status').html('');
				} else {
					$container.html('');
				}
				return false;
			...
			.on( 'click', '.add-row-button', function(e) {
				...
				var input = $('input.whizzy_datepicker',row);
				...
				// row.find('input:text.whizzy_colorpicker').wpColorPicker();
			...
			// ajax when typing
			.on( 'keyup', '.whizzy_oembed', function(event) {
				// fire our ajax function
				CMB.doAjax($(this), event);
			});

			// ajax on paste
			$('.whizzy_oembed').bind( 'paste', function(e) {
				var pasteitem = $(this);
				...
				setTimeout( function() {
					...
					CMB.spinner( '.postbox table.whizzy_metabox', true );
				}, 2000);
			});
		 ...
		 */
		log: function() {
			if ( window.whizzy_l10.script_debug && console && typeof console.log === 'function' ) {
		...
		doAjax: function(obj, e) {
			...
			if (e === 'paste' || e.which <= 90 && e.which >= 48 || e.which >= 96 && e.which <= 111 || e.which == 8 || e.which == 9 || e.which == 187 || e.which == 190) {
				...
				var context = obj.parents('.whizzy_metabox tr td');
				...
				setTimeout( function() {
					...
					if ($('.whizzy_oembed:focus').val() != oembed_url)
						return;
					$.ajax({
						type : 'post',
						dataType : 'json',
						url : window.whizzy_l10.ajaxurl,
						data : {
							'action': 'whizzy_oembed_handler',
							'oembed_url': oembed_url,
							'oembed_width': oembed_width > 300 ? oembed_width : 300,
							'field_id': field_id,
							'object_id': obj.data('objectid'),
							'object_type': obj.data('objecttype'),
							'whizzy_ajax_nonce': window.whizzy_l10.ajax_nonce
						},
		...
		resizeoEmbeds: function() {
			$('table.whizzy_metabox').each( function( index ) {
				var self = $(this);
