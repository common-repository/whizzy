...
class whizzy_Meta_Box_ajax {
	 ...
	 * @since  0.1.0
	 * @return whizzy_Meta_Box_ajax A single instance of this class.
	 */
	...
	public function oembed_handler() {
		...
		// verify our nonce
		if ( ! ( isset( $_REQUEST['whizzy_ajax_nonce'], $_REQUEST['oembed_url'] ) && wp_verify_nonce( $_REQUEST['whizzy_ajax_nonce'], 'ajax_nonce' ) ) )
			die();
		...
		if ( empty( $oembed_string ) )
			self::send_result( '<p class="ui-state-error-text">'. esc_html__( 'Please Try Again', 'whizzy' ) .'</p>', false );
		...
		$html = self::get_oembed( $oembed_url, intval( $_REQUEST['object_id'] ), array(
	...
	public static function get_oembed( $url, $object_id, $args = array() ) {
		...
		if ( isset( $args['object_type'] ) && $args['object_type'] != 'post' ) {
			...
			add_filter( 'get_post_metadata', array( 'whizzy_Meta_Box_ajax', 'hijack_oembed_cache_get' ), 10, 3 );
			// Sets ombed cache in our object's meta (vs postmeta)
			add_filter( 'update_post_metadata', array( 'whizzy_Meta_Box_ajax', 'hijack_oembed_cache_set' ), 10, 4 );
		...
		if ( $check_embed && $check_embed != $fallback )
			return '<div class="embed_status">'. esc_html( $check_embed ) .'<p><a href="#" class="whizzy_remove_file_button" rel="'. esc_attr( $args['field_id'] ) .'">'. esc_html__( 'Remove Embed', 'whizzy' ) .'</a></p></div>';

		// Otherwise, send back error info that no oEmbeds were found
		return '<p class="ui-state-error-text">'. sprintf( esc_html__( 'No oEmbed Results Found for %s. View more info at', 'whizzy' ), $fallback ) .' <a href="http://codex.wordpress.org/Embeds" target="_blank">' . esc_html( 'codex.wordpress.org/Embeds' ) . '</a>.</p>';
	...
	public static function hijack_oembed_cache_get( $check, $object_id, $meta_key ) {
		...
		$data = 'options-page' === self::$object_type
			? whizzy_Meta_Box::get_option( self::$object_id, self::$embed_args['cache_key'] )
	...
	public static function hijack_oembed_cache_set( $check, $object_id, $meta_key, $meta_value ) {
		...
		if ( 'options-page' === self::$object_type ) {
			// Set the option
			whizzy_Meta_Box::update_option( self::$object_id, self::$embed_args['cache_key'], $meta_value, array( 'type' => 'oembed' ) );
			// Save the option
			whizzy_Meta_Box::save_option( self::$object_id );
		} else {
