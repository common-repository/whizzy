 ...
 * @since  0.0.4
 */
class whizzy_Meta_Box_Sanitize {

	/**
	 * A single instance of this class.
	 * @var   whizzy_Meta_Box_Sanitize object
	 * @since 1.0.0
	 */
	 ...
	 * @since  1.0.0
	 * @return whizzy_Meta_Box_Sanitize A single instance of this class.
	 */
	...
	/**
	 * Because we don't want oembed data passed through the whizzy_validate_ filter
	 * @since  1.0.0
	...
	public static function text_datetime_timestamp( $meta ) {
		...
		if ( $tz_offset = whizzy_Meta_Box::field_timezone_offset() )
			$meta += $tz_offset;
	...
	public static function text_datetime_timestamp_timezone( $meta ) {
		...
		if ( empty( $tzstring ) )
			$tzstring = whizzy_Meta_Box::timezone_string();

		$offset = whizzy_Meta_Box::timezone_offset( $tzstring, true );
	...
	public static function file( $meta, $field ) {
		...
		// get _id old value
		$_id_old = whizzy_Meta_Box::get_data( $_id_name );
		...
		if ( isset( $field['save_id'] ) && ! $field['save_id'] ) {
			...
			if ( isset( $_POST[ $field['id'] ] ) && $_POST[ $field['id'] ] && ! $_new_id ) {
				$_new_id = whizzy_Meta_Box::image_id_from_url( esc_url_raw( $_POST[ $field['id'] ] ) );
			}
		...
		if ( $_new_id && $_new_id != $_id_old ) {
			$updated[] = $_id_name;
			whizzy_Meta_Box::update_data( $_new_id, $_id_name );
		} elseif ( '' == $_new_id && $_id_old ) {
			$updated[] = $_id_name;
			whizzy_Meta_Box::remove_data( $_id_name, $old );
		}
	...
	public static function default_sanitization( $meta_value, $field ) {
		...
		$object_type = whizzy_Meta_Box::get_object_type();
		$object_id   = whizzy_Meta_Box::get_object_id();

		// Allow field type validation via filter
		$updated     = apply_filters( 'whizzy_validate_'. $field['type'], null, $meta_value, $object_id, $field, $object_type );
		...
		switch ( $field['type'] ) {
			case 'wysiwyg':
				...
				// break;
			case 'textarea_small':
				$cb = array( 'whizzy_Meta_Box_Sanitize', 'textarea' );
				break;
			default:
