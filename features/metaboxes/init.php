...
spl_autoload_register('whizzy_Meta_Box::autoload_helpers');

$meta_boxes = array();
$meta_boxes = apply_filters( 'whizzy_meta_boxes', $meta_boxes );
foreach ( $meta_boxes as $meta_box ) {
	$my_box = new whizzy_Meta_Box( $meta_box );
}

define( 'WHIZZY_META_BOX_URL', whizzy_Meta_Box::get_meta_box_url() );

/**
 * Create meta boxes
 */
class whizzy_Meta_Box {
	...
	protected static $mb_defaults = array(
		...
		'whizzy_styles' => true, // Include cmb bundled stylesheet
		'fields'     => array(),
	);
	...
	function __construct( $meta_box ) {
		...
		$allow_frontend = apply_filters( 'whizzy_allow_frontend', true, $meta_box );
		...
		$show_filters = 'whizzy_Meta_Box_Show_Filters';
		foreach ( get_class_methods( $show_filters ) as $filter ) {
			add_filter( 'whizzy_show_on', array( $show_filters, $filter ), 10, 2 );
		}
	...
	public function register_scripts() {
		...
		if ( 3.5 <= $wp_version ) {
			...
			if ( ! is_admin() ) {
				...
				wp_localize_script( 'wp-color-picker', 'wpColorPickerL10n', array(
					'clear' => esc_html__( 'Clear', 'whizzy' ),
					'defaultString' => esc_html__( 'Default', 'whizzy' ),
					'pick' => esc_html__( 'Select Color', 'whizzy' ),
					'current' => esc_html__( 'Current Color', 'whizzy' ),
				) );
			}
		} else {
		...
		wp_register_script( 'cmb-timepicker', WHIZZY_META_BOX_URL . 'js/jquery.timePicker.min.js' );
		wp_register_script( 'cmb-scripts2', WHIZZY_META_BOX_URL .'js/cmb'. $min .'.js', $scripts, self::CMB_VERSION );
		wp_register_script( 'whizzy_gallery', WHIZZY_META_BOX_URL . 'js/whizzy_gallery.js' );
		wp_enqueue_media();

		wp_localize_script( 'cmb-scripts2', 'whizzy_l10', array(
			'ajax_nonce'      => wp_create_nonce( 'ajax_nonce' ),
			'script_debug'    => defined('SCRIPT_DEBUG') && SCRIPT_DEBUG,
			'new_admin_style' => version_compare( $wp_version, '3.7', '>' ),
			'object_type'     => self::get_object_type(),
			'upload_file'     => esc_html__( 'Use this file', 'whizzy' ),
			'remove_image'    => esc_html__( 'Remove Image', 'whizzy' ),
			'remove_file'     => esc_html__( 'Remove', 'whizzy' ),
			'file'            => esc_html__( 'File:', 'whizzy' ),
			'download'        => esc_html__( 'Download', 'whizzy' ),
			'ajaxurl'         => admin_url( '/admin-ajax.php' ),
		) );

		wp_register_style( 'whizzy-styles', WHIZZY_META_BOX_URL . 'style.css', $styles );
	...
	public function do_scripts( $hook ) {
		...
		if ( $hook == 'post.php' || $hook == 'post-new.php' || $hook == 'page-new.php' || $hook == 'page.php' ) {
			...
			if ( get_post_type() === 'whizzy_proof_gallery' ) {
				...
				if ( $this->_meta_box['whizzy_styles'] != false )
					wp_enqueue_style( 'whizzy-styles' );
	...
	public function add_metaboxes() {
		...
		foreach ( $this->_meta_box['pages'] as $page ) {
			if ( apply_filters( 'whizzy_show_on', true, $this->_meta_box ) )
	...
	public static function show_form( $meta_box, $object_id = 0, $object_type = '' ) {
		...
		// get box types
		$types = whizzy_Meta_Box_types::get();

		// Use nonce for verification
		echo "\n<!-- Begin CMB Fields -->\n";
		wp_nonce_field( self::nonce(), 'wp_meta_box_nonce', false, true );
		do_action( 'whizzy_before_table', $meta_box, $object_id, $object_type );

		echo '<div class="tabs">
			<div class="tabs-header">
				<ul>
					<li class="active"><a href="#"><span class="dashicons-before dashicons-images-alt2"></span>' . esc_html__( 'Gallery', 'whizzy' ) . '</a></li>
					<li><a href="#"><span class="dashicons-before dashicons-admin-generic"></span>' . esc_html__( 'Settings','whizzy' ) . '</a></li>
					<li><a href="#"><span class="dashicons-before dashicons-chart-line"></span>' . esc_html__( 'Activities','whizzy' ) . '</a></li>
					<li><a href="#"><span class="dashicons-before dashicons-email"></span>' . esc_html__( 'Send Email','whizzy ') . '</a></li>
				</ul>
			</div>
			<div class="tabs-content">
				<div class="tabs-item active">';
					foreach ( $meta_box['fields']['tab_one'] as $field ) {
						...
						$field['default'] = apply_filters( 'whizzy_default_filter', $field['default'], $field, $object_id, $object_type );
						// 'whizzy_std_filter' deprectated, use 'whizzy_default_filter' instead
						$field['default'] = apply_filters( 'whizzy_std_filter', $field['default'], $field, $object_id, $object_type );
						...
						// (assuming no one would want to save 'whizzy_no_override_val' as a value)
						$meta = apply_filters( 'whizzy_override_meta_value', 'whizzy_no_override_val', $object_id, $field, $object_type );

						// If no override, get our meta
						if ( $meta === 'whizzy_no_override_val' )
							$meta = self::get_data();
						...
						echo '<table><tr class="cmb-type-'. sanitize_html_class( $field['type'] ) .' whizzy_id_'. sanitize_html_class( $field['id'] ) . $classes .'">';
						...
						if ( $field['type'] == "title" || $field['type'] == "gallery" ) {
							...
							if ( isset( $meta_box['show_names'] ) && $meta_box['show_names'] == true && $field['type'] != 'gallery' ) {
								...
								echo '<th'. $style .'><label for="', esc_attr( $field['id'] ), '">', esc_html( $field['name'] ), '</label></th>';
							} else {
								echo '<label style="display:none;" for="', esc_attr( $field['id'] ), '">', esc_html( $field['name'] ), '</label></th>';
							}
							echo '<td>';
						}
				...
				echo '<div class="tabs-item">';
					foreach ( $meta_box['fields']['tab_two'] as $field ) {
				...
				echo '<div class="tabs-item">';
					...
					if (isset($_GET['post'])) {
						...
						echo '<tr>';
						echo '<td>';
						echo  __( 'Filename', 'whizzy' );
						echo '</td>';
						echo '<td>';
						echo  __( 'Name', 'whizzy' );
						echo '</td>';
						echo '<td>';
						echo  __( 'Downloads ZIP', 'whizzy' );
						echo '</td>';
						echo '<td>';
						echo  __( 'Downloads PDF', 'whizzy' );
						echo '</td>';
						echo '</tr>';
						...
						foreach ( $attachments as $key => $attachment ) {
							...
							echo '<tr>';
							echo '<td>';
							echo '<img src="' . esc_url( $img_url[0] ) . '" alt="">';
							echo '</td>';
							echo '<td>';
							echo esc_html( $filename );
							echo '</td>';
							echo '<td>';
							echo '<span class="wp-menu-image dashicons-before dashicons-arrow-down-alt"></span> ';
							echo !empty( $current_count ) ? esc_html( $current_count ) : esc_html('0');
							echo __( ' downloads', 'whizzy' );
							echo '</td>';
							echo '<td>';
							echo '<span class="wp-menu-image dashicons-before dashicons-arrow-down-alt"></span> ';
							echo !empty( $current_count_pdf ) ? esc_html( $current_count_pdf ) : esc_html('0');
							echo __( ' downloads', 'whizzy' );
							echo '</td>';
							echo '</tr>';
				...
				<div class="tabs-item">';
					...
					echo '<div class="whizzy-send-email" data-url="' . esc_attr( $post_url ) . '" data-site="'. esc_attr( get_bloginfo() ) .'">
							<label>
								' . esc_html__( 'To', 'whizzy' ) . '
								<input type="email" name="whizzy_email" placeholder="' . esc_html__( 'Email', 'whizzy' ) . '">
							</label>
							<label>
								' . esc_html__( 'Subject', 'whizzy' ) . '
								<input type="text" name="whizzy_subject" placeholder="' . esc_html__( 'Your subject', 'whizzy' ) . '">
							</label>
							<label>
								' . esc_html__( 'Message', 'whizzy' ) . '
								<textarea name="whizzy_message" cols="30" rows="10" placeholder="' . esc_html__( 'Your message', 'whizzy' ) . '"></textarea>
							</label>
							<div class="button-wrap">
							<button id="whizzy-send">' . esc_html__( 'Send', 'whizzy' ) . '</button></div>
							<p class="done">' . esc_html__( 'Your message is sent!', 'whizzy' ) . '</p>
							<p class="error">' . esc_html__( "Oooops! Your message isn't sent!", "whizzy" ) . '</p>
							</div>';
				echo '</div>
			</div>
		</div>';
		do_action( 'whizzy_after_table', $meta_box, $object_id, $object_type );
		echo "\n<!-- End CMB Fields -->\n";
	...
	public static function save_fields( $meta_box, $object_id, $object_type = '' ) {
		...
		if ( ! apply_filters( 'whizzy_show_on', true, $meta_box ) )
			return;

		// save field ids of those that are updated
		$updated = array();

		foreach ( $meta_box['fields'] as $item ) {
			foreach ( $item as $field ) {
				...
				if ( $object_type == 'post' ) {
					if ( isset( $field['taxonomy'] ) && in_array( $field['type'], array( 'taxonomy_select', 'taxonomy_radio', 'taxonomy_multicheck' ) ) ) {
		...
		do_action( "whizzy_save_{$object_type}_fields", $object_id, $meta_box['id'], $updated, $meta_box );
	...
	public static function get_object_id( $object_id = 0 ) {
		...
		switch ( self::get_object_type() ) {
			case 'user':
				$object_id = isset( $GLOBALS['user_ID'] ) ? intval( $GLOBALS['user_ID'] ) : $object_id;
				$object_id = isset( $_REQUEST['user_id'] ) ? intval( $_REQUEST['user_id'] ) : $object_id;
				break;

			default:
				$object_id = isset( $GLOBALS['post']->ID ) ? intval( $GLOBALS['post']->ID ) : $object_id;
				$object_id = isset( $_REQUEST['post'] ) ? intval( $_REQUEST['post'] ) : $object_id;
				break;
		}
	...
	public static function sanitization_cb( $meta_value, $field = array() ) {
		...
		// Validation via 'whizzy_Meta_Box_Sanitize' (with fallback filter)
		return call_user_func( array( whizzy_Meta_Box_Sanitize::get(), $field['type'] ), $meta_value, $field );
	}
	...
	public static function maybe_callback( $field, $cb ) {
		...
		if ( ( is_string( $cb ) && function_exists( $cb ) ) || ( is_array( $cb ) && is_callable( $cb ) ) ) {
			return $cb;
		}
	...
	public static function get_meta_box_url() {
		...
		if ( strtoupper( substr( PHP_OS, 0, 3 ) ) === 'WIN' ) {
			...
			$whizzy_url = str_replace( DIRECTORY_SEPARATOR, '/', $content_url );

		} else {
			$whizzy_url = str_replace(
		...
		return trailingslashit( apply_filters( 'whizzy_meta_box_url', $whizzy_url ) );
	}
	 ...
	 * @uses apply_filters() Calls 'whizzy_override_option_get_$option_key' hook to allow
	...
	public static function _get_option( $option_key, $default = false ) {
		...
		$test_get = apply_filters( "whizzy_override_option_get_$option_key", 'whizzy_no_override_option_get', $default );

		if ( $test_get !== 'whizzy_no_override_option_get' )
			return $test_get;
	 ...
	 * @uses apply_filters() Calls 'whizzy_override_option_save_$option_key' hook to allow
	...
	public static function save_option( $option_key ) {
		...
		$test_save = apply_filters( "whizzy_override_option_save_{$option_key}", 'whizzy_no_override_option_save', $to_save );

		if ( $test_save !== 'whizzy_no_override_option_save' )
			return $test_save;
...
// Handle oembed Ajax
add_action( 'wp_ajax_whizzy_oembed_handler', array( 'whizzy_Meta_Box_ajax', 'oembed_handler' ) );
add_action( 'wp_ajax_nopriv_whizzy_oembed_handler', array( 'whizzy_Meta_Box_ajax', 'oembed_handler' ) );
 ...
 * @return array               Options array or specific field
 */
function whizzy_get_option( $option_key, $field_id = '' ) {
	return whizzy_Meta_Box::get_option( $option_key, $field_id );
}
 ...
 * @param int   $object_id  Object ID
 */
function whizzy_print_metaboxes( $meta_boxes, $object_id ) {
	foreach ( (array) $meta_boxes as $meta_box ) {
		whizzy_print_metabox( $meta_box, $object_id );
	}
}
 ...
 * @param int   $object_id Object ID
 */
function whizzy_print_metabox( $meta_box, $object_id ) {
	$cmb = new whizzy_Meta_Box( $meta_box );
	if ( $cmb ) {

		whizzy_Meta_Box::set_object_id( $object_id );
		...
		if ( $meta_box['whizzy_styles'] != false )
			wp_enqueue_style( 'whizzy-styles' );

		whizzy_Meta_Box::show_form( $meta_box );
	}
 ...
 * @param int   $object_id Object ID
 */
function whizzy_save_metabox_fields( $meta_box, $object_id ) {
	whizzy_Meta_Box::save_fields( $meta_box, $object_id );
}
 ...
 */
function whizzy_metabox_form( $meta_box, $object_id, $echo = true ) {

	$meta_box = whizzy_Meta_Box::set_mb_defaults( $meta_box );
	...
	whizzy_Meta_Box::set_object_type( whizzy_Meta_Box::set_mb_type( $meta_box ) );
	...
	if (
		...
		&& wp_verify_nonce( $_POST['wp_meta_box_nonce'], whizzy_Meta_Box::nonce() )
		&& $_POST['object_id'] == $object_id
	)
		whizzy_save_metabox_fields( $meta_box, $object_id );
	...
	$form_format = apply_filters( 'whizzy_frontend_form_format', '<form class="cmb-form" method="post" id="%s" enctype="multipart/form-data" encoding="multipart/form-data"><input type="hidden" name="object_id" value="%s">%s<input type="submit" name="submit-cmb" value="%s" class="button-primary"></form>', $object_id, $meta_box, $form );

	$form = sprintf( $form_format, $meta_box['id'], $object_id, $form, esc_html__( 'Save', 'whizzy' ) );
...
function ajax_whizzy_pixgallery_preview(){
	...
	foreach ( $ids as $id ) {
		$attach = wp_get_attachment_image_url( intval( $id ), 'thumbnail', false);

		if ( $attach ) {
			$result["output"] .= '<li><img src="'. esc_url( $attach ) .'" /></li>';
		}
	}
...
add_action( 'wp_ajax_ajax_whizzy_pixgallery_preview', 'ajax_whizzy_pixgallery_preview' );
